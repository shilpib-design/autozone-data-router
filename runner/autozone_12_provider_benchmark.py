...
def textify(v:Any)->str: return v if isinstance(v,str) else json.dumps(v,ensure_ascii=False)
def clean(s:str)->str:
...
def run_case(case):
    u=case["url"]; out=[]
    if os.getenv("SCRAPE_DO_API_KEY"): out.append(call("scrape.do",case,"GET","https://api.scrape.do/",params={"token":os.environ["SCRAPE_DO_API_KEY"],"url":u,"super":"true","geoCode":"us","postalcode":"90001","render":"true","waitUntil":"networkidle2"}))
    if os.getenv("SCRAPINGBEE_API_KEY"): out.append(call("scrapingbee",case,"GET","https://app.scrapingbee.com/api/v1/",headers={"Authorization":f"Bearer {os.environ['SCRAPINGBEE_API_KEY']}"},params={"url":u,"mode":"auto","render_js":"true","premium_proxy":"true","country_code":"us"}))
    if os.getenv("SCRAPERAPI_API_KEY"): out.append(call("scraperapi",case,"GET","https://api.scraperapi.com/",params={"api_key":os.environ["SCRAPERAPI_API_KEY"],"url":u,"country_code":"us","render":"true","premium":"true"}))
    if os.getenv("SCRAPEHERO_API_KEY"): out.append(call("scrapehero",case,"GET","https://get.scrapehero.com/autozone/product-details/",params={"x-api-key":os.environ["SCRAPEHERO_API_KEY"],"url":u,"zipcode":"90001"}))
    if os.getenv("ZENROWS_API_KEY"): out.append(call("zenrows",case,"GET","https://api.zenrows.com/v1/",params={"apikey":os.environ["ZENROWS_API_KEY"],"url":u,"js_render":"true","premium_proxy":"true","proxy_country":"us","wait":"3000"}))
    if os.getenv("FIRECRAWL_API_KEY"): out.append(call("firecrawl",case,"POST","https://api.firecrawl.dev/v2/scrape",headers={"Authorization":f"Bearer {os.environ['FIRECRAWL_API_KEY']}","Content-Type":"application/json"},json_body={"url":u,"formats":["markdown","html"],"waitFor":5000,"location":{"country":"US","languages":["en-US"]},"proxy":"auto","timeout":120000}))
    ...
    if os.getenv("SCRAPINGANT_API_KEY"): out.append(call("scrapingant",case,"GET","https://api.scrapingant.com/v2/general",headers={"x-api-key":os.environ["SCRAPINGANT_API_KEY"]},params={"url":u,"browser":"true","return_page_source":"true"}))
    if os.getenv("SPIDERCLOUD_API_KEY"): out.append(call("spidercloud",case,"POST","https://api.spider.cloud/scrape",headers={"Authorization":f"Bearer {os.environ['SPIDERCLOUD_API_KEY']}","Content-Type":"application/json"},json_body={"url":u,"return_format":"markdown"}))
    ...
    if os.getenv("SOCIALCRAWL_API_KEY"): out.append(make_result("socialcrawl",case,None,0,capability="not_applicable",error="SocialCrawl is a social-media data API; AutoZone web scraping is outside its documented capability."))
    return out

def main():
    results=[]
    # 12 providers are executed concurrently per product to keep the benchmark fast while preserving identical test inputs.
    with ThreadPoolExecutor(max_workers=12) as pool:
        futures=[pool.submit(run_case,case) for case in CONFIG["tests"]]
        for f in as_completed(futures): results.extend(f.result())
    results.sort(key=lambda r:(r["provider"],r["test_id"]))
